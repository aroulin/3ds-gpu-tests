...
// Testing the MOVA shader instruction

// For each test:
// Draws a 1px by 1px quad in the bottom-left corner using a vertex shader
// The MOVA instruction is used to output the color of this quad.
// The framebuffer is then read to verify the results

// exact shader instructions under tests:
/*
	mov r0, zeros
	mov r1, ones

	mova src1_uniform
	mov outclr.xyz, r0[a0]
	mov outclr.w, ones
*/
...
static void Test_MOVA_RoundsTowardsZero(void) {
	printf("Test: MOVA_RoundsTowardsZero\n");
	src1_uniform.x = 0.9f, expected_result.x = 0.0f;
	src1_uniform.y = 0.9f, expected_result.y = 0.0f;
	src1_uniform.z = 0.9f, expected_result.z = 0.0f;
	src1_uniform.w = 0.9f;
}

static test_t tests[] = {
	&Test_MOVA_RoundsTowardsZero
};
